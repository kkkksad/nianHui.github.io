<template>
  <div id="root">
    ...
    <Tool @toggle="toggle" @resetConfig="reloadTagCanvas" @getPhoto="getPhoto" :running="running" :closeRes="closeRes" />
...
export default {
  ...
  computed: {
    ...
    datas() {
      // 从导入的 list 和 photos 中生成数据
      return this.list.map((item) => {
        const photo = this.photos.find((d) => d.id === item.key);
        return {
          key: item.key,
          name: item.name || '',
...
#root {
  height: 100%;
  position: relative;
  background-image: url('./assets/loctek-img1.jpg');
